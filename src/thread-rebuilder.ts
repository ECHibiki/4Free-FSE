...
enhance4ChanX():void{
	...
	help_icon_container.addEventListener("click", (ev)=>{
	...
	var xhr = new GM_xmlhttpRequest(({
		...
		onload: function(data){
			...
			if(data == undefined){
				...
				if(this.use_offsite_archive) data["posts"] = Object.values(data["posts"]);
				...
				for(var post_number = starting_post ; post_number < len ; post_number++){
					...
					if(filename !== undefined && filename !== null && filename.indexOf("undefined") == -1)
						if(this.use_offsite_archive)
							...
							thread_data[1].push("https://i.4cdn.org/" + this.board + "/" + filename);
					else  thread_data[1].push("");
