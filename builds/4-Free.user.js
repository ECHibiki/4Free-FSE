...
var DanbooruImageAdder = /** @class */ (function (_super) {
    ...
    function DanbooruImageAdder() {
        ...
        _this.subdomain_regex = new RegExp("(raikou|hijiribe)\d*\.");
        _this.time_max = 10;
        _this.time = time_max;
        //check if valid url location
        _this.primed_for_fail = false;
        _this.init();
        _this.activate();
        return _this;
    }
    ...
    DanbooruImageAdder.prototype.enhanced4ChanXListeners = function () {
        ...
        tag_input.addEventListener("input", function (evt) {
            _this.setTagInterface(second_row_nodes[1], auto_complete_row);
        });
        second_row_nodes[2].addEventListener("click", function (evt) { return _this.buttonClickFunction; });
    };
    ...
    DanbooruImageAdder.prototype.setImage = function () {
        //Set image tags.
        ...
        var xhr_image_load = new GM_xmlhttpRequest(({
            ...
            onload: function (data) {
                ...
                if (failed_to_find_required_tags_state)
                    return;
                //set the end
    ...
    DanbooruImageAdder.prototype.ratingURL = function (tags, data) {
        var URL = "";
        ...
        if (document.getElementById("safe").checked) {
            if (document.getElementById("questionable").checked) {
                if (document.getElementById("explicit").checked) {
                    if (data.length > 1)
                        URL = "&utf8=%E2%9C%93&tags=" + data[data.length - 2]["name"] + "+" + data[data.length - 1]["name"];
                    else
                        URL = "&utf8=%E2%9C%93&tags=" + data[data.length - 1]["name"];
                }
                else {
    ...
    //set where to search
    DanbooruImageAdder.prototype.setPostAndPage = function (end_URL, tags) {
        //posts
        if (this.post_number > 0)
            this.post_number = 0;
        //page
        if (top_page != top_page_max)
            smallest_tag_size = top_page * 20;
        if (smallest_tag_size == 0)
            smallest_tag_size = 100;
        do {
            escape_cond = true;
    ...
    DanbooruImageAdder.prototype.checkPageFromDanbooru = function (err, data, tags) {
        if (err != null) {
    ...
    DanbooruImageAdder.prototype.setImageFromDanbooru = function (err, data, tags) {
        var _this = this;
        if (err != null) {
            console.log('Something went wrong: ' + err);
            alert4ChanX("Danbooru Server Did Not Perform request -- Error: " + err, "error");
            ...
            for (this.post_number = this.post_number; this.post_number < 20; this.post_number++) {
                ...
                var end_URL = this.json_page["" + this.post_number].file_url;
                ...
                if (end_URL === undefined ||
                    ...
                    tags.forEach(function (tag) {
                        ...
                        if (tag.indexOf("order:") > -1)
                            ...
                            if (tag.charAt(7) !== json_page["" + _this.post_number]["rating"]) {
                                failed_to_find_required_tags = true;
                            }
                        }
                        else if (json_page["" + _this.post_number]["tag_string"].indexOf(tag) == -1) {
                            failed_to_find_required_tags = true;
                        }
                    });
                }
                if (failed_to_find_required_tags) {
                    ...
                    var xhr = new GM_xmlhttpRequest(({
                        ...
                        onload: function (response) {
                            ...
                            _this.reset_search_timer_fields();
                            clearInterval(_this.intervalFunction);
                            _this.time = _this.time_max;
